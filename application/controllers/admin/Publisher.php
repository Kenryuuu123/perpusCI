...
class Penerbit extends CI_Controller {
	...
	public function index()
    {
        ...
        {
            /*layout*/  
            $data['title']='Daftar Publisher';
            $data['pointer']="Publisher";
            $data['classicon']="fa fa-book";
            $data['main_bread']="Data Publisher";
            $data['sub_bread']="Daftar Publisher";
            $data['desc']="Data Master Publisher, Menampilkan data Publisher";

            /*data yang ditampilkan*/
            $data['data_publisher '] = $this->Dokumen_model->getAllData("tb_publisher");
            ...
            $tmp['content']=$this->load->view('admin/publisher/View_publisher',$data, TRUE);
    ...
    public function create()
    {
        ...
        {   
                ...
                if($this->form_validation->run()==FALSE)
                {
                    ...
                    /*data yang ditampilkan*/
                    $data['data_provinsi'] = $this->Dokumen_model->getAllData("tb_provinsi");
                    $data['data_publisher'] = $this->Dokumen_model->getAllData("tb_penerbit");
                    //$data['data_agama'] = $this->Buku_model->getAllData("tb_agama");
                    //$data['isi']=$this->Anggota_model->get_all();
                    //$data['js']=$this->load->view('admin/anggota/js');
                    $tmp['content']=$this->load->view('admin/penerbit/Create_publisher',$data, TRUE);
                    $this->load->view('admin/layout',$tmp);
                }
                else
                {
                    $data = array('id_publisher' => '',
                                  'nama_publisher' => $this->input->post('publisher')
                                );
                    $quer=$this->Buku_model->insertData('tb_publisher',$data);
                    redirect("admin/Publisher","refresh");   
                }
        }
        else
        {
    ...
    { 
        ...
        if(!empty($cek) && $stts=='admin')
        {       
                $id = $this->input->get('id_publisher',true);    
                $this->form_validation->set_rules('penerbit', 'penerbit', 'trim|required');
                $this->form_validation->set_rules('provinsi', 'provinsi', 'trim|required');
                if($this->form_validation->run()==FALSE)
                {
